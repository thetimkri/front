...
// `;
//
// export const UploadFile = styled.div<{ $hasError: boolean; $isMultiple: boolean }>`
//   display: flex;
//   align-items: center;
//   gap: 8px;
//   width: ${(props) => (props.$isMultiple ? '100%' : 'max-content')};
//   max-width: 312px;
//   padding: 12px 16px;
//   border: 1px solid ${(props) => (props.$hasError ? `${colors.error}` : `${colors.success}`)};
//   border-radius: 40px;
//   ${(props) => (props.$isMultiple ? '' : 'margin-top: 20px')};
//
